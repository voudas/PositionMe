...
public class PdrProcessing {
    ...
    private static final float movementThreshold = 0.4f; // m/s^2
    // Threshold under which movement is considered non-existent
    private static final float epsilon = 0.18f;
    //endregion

    private float X = 0; // Filtered value
    private float P = 1; // Estimation error covariance
    ...
    public float[] updatePdr(long currentStepEnd, List<Double> accelMagnitudeOvertime, float headingRad) {
        // Change angle so zero rad is east and positive rotation is counter-clockwise
        float adaptedHeading = (float) (Math.PI / 2 - headingRad);

        // Normalize heading to be between 0 and 2 * PI
        adaptedHeading = (adaptedHeading + (float)(2 * Math.PI)) % (float)(2 * Math.PI);

        // Calculate step length
        if (!useManualStep) {
            float avgAccelMagnitude = calculateAverageMagnitude(accelMagnitudeOvertime);
            this.stepLength = calculateDynamicStepLength(avgAccelMagnitude);
        }

        // Translate to cartesian coordinate system
        float deltaX = (float) (stepLength * Math.cos(adaptedHeading));
        float deltaY = (float) (stepLength * Math.sin(adaptedHeading));
    ...
    private float calculateDynamicStepLength(float accelMagnitude) {
        // Constants for tuning
        float stepLengthBase = .55f; // Base step length in meters
        float stepLengthFactor = 0.05f; // Factor to adjust step length based on accel magnitude

        // Update acceleration magnitude using Kalman Filter
        updateKalman(accelMagnitude);

        // Dynamic step length calculation
        return stepLengthBase + (X * stepLengthFactor);
    }
    /**
     * Updates step length for each step based on accelerometer data.
     *
     * @param accelMagnitudeOvertime List<Double> containing accelerometer magnitude over time.
     */
    public void updateStepLength(List<Double> accelMagnitudeOvertime) {
        float avgAccelMagnitude = calculateAverageMagnitude(accelMagnitudeOvertime);
        float currentStepLength = calculateDynamicStepLength(avgAccelMagnitude);

        // Adjust step length based on elevation
        currentStepLength *= adjustStepLengthForElevation();

        // Update total step length and count
        sumStepLength += currentStepLength;
        stepCount++;
    }
    // Adjust step length based on elevation
    private float adjustStepLengthForElevation() {
        // Example adjustments - these values need empirical tuning
        float uphillAdjustment = 0.9f;
        float downhillAdjustment = 1.1f;
        float elevationChangeThreshold = 5.0f; // meters

        if (elevation > elevationChangeThreshold) {
            return uphillAdjustment;
        } else if (elevation < -elevationChangeThreshold) {
            return downhillAdjustment;
        }
        return 1.0f; // No adjustment
    }
    private float calculateAverageMagnitude(List<Double> magnitudes) {
        double sum = 0;
        for (Double magnitude : magnitudes) {
            sum += magnitude;
        }
        return (float) (sum / magnitudes.size());
    }
    private void updateKalman(float value) {
        // Process noise
        float Q = 0.001f;
        // Measurement noise
        float R = 0.001f;
