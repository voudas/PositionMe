    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        switch (sensorEvent.sensor.getType()) {
            case Sensor.TYPE_ACCELEROMETER:
                // Accelerometer processing apply lpf
                acceleration[0] = ALPHA * acceleration[0] + (1 - ALPHA) * sensorEvent.values[0];
                acceleration[1] = ALPHA * acceleration[1] + (1 - ALPHA) * sensorEvent.values[1];
                acceleration[2] = ALPHA * acceleration[2] + (1 - ALPHA) * sensorEvent.values[2];
                break;
            ...
            case Sensor.TYPE_STEP_DETECTOR:
                ...
                float[] newCords = pdrProcessing.updatePdr(stepTime, accelMagnitude, orientation[0]);

                // Update step length for each step
                pdrProcessing.updateStepLength(this.accelMagnitude);
